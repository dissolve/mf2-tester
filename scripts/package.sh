...
for f in results/test-results/*/*/*.json ; 
    do 
        ...
        echo '<tr>' >> results/index.html;
        echo '<td><a href="'$TEST'">'$NAME'</a></td>' >> results/index.html;

        RESULT_MD5=`md5sum results/$RESULT |cut -d ' ' -f 1`;
        SHORT_MD5=`echo $RESULT_MD5 |cut -c 1-10`
        CLASS="pass";
        echo "<td class='$CLASS'>Test Suite: <a href='$RESULT'>View</a><br><span class='md5' title='$RESULT_MD5'>$SHORT_MD5...</span></td>" >> results/index.html;
        TOTAL=$[$TOTAL + 1]
        ...
        for lang in `ls languages`; do 
            ...
            if [ "$RESULT_MD5" = "$lang_RESULT_MD5" ]; then
                echo "<td class='pass'>Result: <a href='$lang_RESULT'>View</a> <br><span class='md5' title='$lang_RESULT_MD5'>$lang_SHORT_MD5...</span></td>" >> results/index.html;
